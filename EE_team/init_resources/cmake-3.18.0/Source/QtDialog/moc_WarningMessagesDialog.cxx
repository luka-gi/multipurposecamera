/****************************************************************************
** Meta object code from reading C++ file 'WarningMessagesDialog.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "WarningMessagesDialog.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'WarningMessagesDialog.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_WarningMessagesDialog[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      22,   33,   33,   33, 0x08,
      34,   74,   33,   33, 0x08,
      80,   74,   33,   33, 0x08,
     121,   74,   33,   33, 0x08,
     161,   74,   33,   33, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_WarningMessagesDialog[] = {
    "WarningMessagesDialog\0doAccept()\0\0"
    "doSuppressDeveloperWarningsChanged(int)\0"
    "state\0doSuppressDeprecatedWarningsChanged(int)\0"
    "doDeveloperWarningsAsErrorsChanged(int)\0"
    "doDeprecatedWarningsAsErrorsChanged(int)\0"
};

void WarningMessagesDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        WarningMessagesDialog *_t = static_cast<WarningMessagesDialog *>(_o);
        switch (_id) {
        case 0: _t->doAccept(); break;
        case 1: _t->doSuppressDeveloperWarningsChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->doSuppressDeprecatedWarningsChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->doDeveloperWarningsAsErrorsChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->doDeprecatedWarningsAsErrorsChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData WarningMessagesDialog::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject WarningMessagesDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_WarningMessagesDialog,
      qt_meta_data_WarningMessagesDialog, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &WarningMessagesDialog::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *WarningMessagesDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *WarningMessagesDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_WarningMessagesDialog))
        return static_cast<void*>(const_cast< WarningMessagesDialog*>(this));
    if (!strcmp(_clname, "Ui_MessagesDialog"))
        return static_cast< Ui_MessagesDialog*>(const_cast< WarningMessagesDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int WarningMessagesDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
